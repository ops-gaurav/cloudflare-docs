...
You can measure your Worker's startup time by deploying it to Cloudflare using [Wrangler](/workers/wrangler/). When you run `npx wrangler@latest deploy`, Wrangler will output the startup time of your Worker in the command-line output, using the [`startup_time_ms` field](/api/operations/worker-script-upload-worker-module) returned by the Cloudflare API.

When you deploy your Worker to Cloudflare using Wrangler, Wrangler returns the startup time
